...
PROJ_DESCRIPTION = "Rename files or dirs via your text editor."
...
def main(argv):
    ...
    cmd = '\"{0}\" {1}'.format(opt["TEXT_EDITOR_PATH"], opt["TEXT_EDITOR_ARGS"].format(LS_FILE_PATH=opt["LS_FILE_PATH"]))
    print(cmd)
    os.system(cmd)
...
def arg_parse(argv):
    parser = argparse.ArgumentParser(description="{description}".format(description=PROJ_DESCRIPTION),
...
def print_version(file=sys.stdout):
    print("\n{name} {ver}\n".format(name=PROJ_NAME, ver=__version__) +
          "{description}\n".format(description=PROJ_DESCRIPTION) +
          "Present by {author}.\n".format(author=__author__), file=file)


def batch(target: str):
    import os.path
    target_abspath = os.path.abspath(target)
    batch_dir_root, _ = os.path.splitext(target_abspath)
    batch_path = batch_dir_root + '.bat'
    text = '@echo off\npython {} %*\npause'.format(target_abspath)
    with open(batch_path, 'w', encoding='utf-8') as f:
        f.write(text)
    print('Successfully make batch file `{}`.'.format(batch_path))
